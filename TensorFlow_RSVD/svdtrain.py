...
def svd(train, test, total):
    ...
    iter_totaltest = dataio.OneEpochIterator([total["user"],
                                         total["item"],
                                         total["rate"]],
                                        batch_size=-1)
        ...
        for i in range(EPOCH_MAX * samples_per_batch):
            ...
            if i  == EPOCH_MAX * samples_per_batch-1:
                for users, items, rates in iter_totaltest:
                    pred_total = sess.run(infer, feed_dict={user_batch: users,
                                                            item_batch: items})
                    #print(users)
                    #print(items)
...
if __name__ == '__main__':
    df_train, df_test, wt = get_data()
    svd(df_train, df_test, wt)
    print("Done!")
